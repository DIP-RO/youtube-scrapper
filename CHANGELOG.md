...
- **Comment model**: `likes`, `reply_count`, `is_pinned`, `is_hearted` now have
  defaults (0, 0, False, False). Previously these were required positional
  arguments, making it cumbersome to create a Comment manually:
  `Comment(comment_id="abc")` now works instead of requiring all 5 fields.
